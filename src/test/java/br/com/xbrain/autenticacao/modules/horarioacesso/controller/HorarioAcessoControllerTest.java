...
public class HorarioAcessoControllerTest {
    ...
    @SneakyThrows
    @WithAnonymousUser
    public void getHorarioAcessoStatus_deveRetornarOK_quandoNaoPassarToken() {
    ...
    @SneakyThrows
    @WithAnonymousUser
    public void getHorarioAcessoStatusByIdSite_deveRetornarOK_quandoNaoPassarToken() {
        mvc.perform(get(URL + "/status/1")
                .accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());

        verify(service).getStatus(eq(1));
    }
