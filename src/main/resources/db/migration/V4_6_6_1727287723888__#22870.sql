...
INSERT INTO CARGO_DEPART_FUNC (ID, FK_CARGO, FK_DEPARTAMENTO, FK_FUNCIONALIDADE) VALUES (
    SEQ_CARGO_DEPART_FUNC.NEXTVAL,
    (SELECT ID FROM CARGO WHERE CODIGO = 'BACKOFFICE_SUPERVISOR_TRATAMENTO_VENDAS'),
    (SELECT ID FROM DEPARTAMENTO WHERE FK_NIVEL = (SELECT ID FROM NIVEL WHERE CODIGO = 'BACKOFFICE_CENTRALIZADO')),
    22663);

INSERT INTO CARGO_DEPART_FUNC (ID, FK_CARGO, FK_DEPARTAMENTO, FK_FUNCIONALIDADE) VALUES (
    SEQ_CARGO_DEPART_FUNC.NEXTVAL,
    (SELECT ID FROM CARGO WHERE CODIGO = 'BACKOFFICE_COORDENADOR_TRATAMENTO_VENDAS'),
    (SELECT ID FROM DEPARTAMENTO WHERE FK_NIVEL = (SELECT ID FROM NIVEL WHERE CODIGO = 'BACKOFFICE_CENTRALIZADO')),
    22663);

INSERT INTO CARGO_DEPART_FUNC (ID, FK_CARGO, FK_DEPARTAMENTO, FK_FUNCIONALIDADE) VALUES (
    SEQ_CARGO_DEPART_FUNC.NEXTVAL,
    (SELECT ID FROM CARGO WHERE CODIGO = 'BACKOFFICE_GERENTE_TRATAMENTO_VENDAS'),
    (SELECT ID FROM DEPARTAMENTO WHERE FK_NIVEL = (SELECT ID FROM NIVEL WHERE CODIGO = 'BACKOFFICE_CENTRALIZADO')),
    22663);
...
-- ADICIONA PERMISSAO PARA USUARIOS JÁ CADASTRADOS PARA O NIVEL MSO E SUB-NIVEL BACKOFFICE CENTRALIZADO
DECLARE
BEGIN
    FOR usuario IN (
